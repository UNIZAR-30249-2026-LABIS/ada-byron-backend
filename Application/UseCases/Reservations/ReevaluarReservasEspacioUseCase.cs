...
/// <summary>
/// PBI-13 (HU-O4): Reevalúa las reservas futuras de un espacio tras cambios de configuración.
/// - Si una reserva Aceptada ya no cumple aforo, reservabilidad u horario → PotencialmenteInvalida.
/// - Si una reserva PotencialmenteInvalida vuelve a cumplir esas reglas → Aceptada.
    ...
    IEdificioConfigRepository configRepo)
{
    ...
    public async Task<int> ExecuteAsync(Espacio espacio)
    {
        ...
        // 2. Reglas actuales del espacio, con el nuevo valor ya persistido
        ...
        var aInvalidar = aceptadas
            .Where(r => !CumpleConfiguracionActual(espacio, r, aforoEfectivo))
            .ToList();
    ...
    private static bool CumpleConfiguracionActual(Espacio espacio, Domain.Aggregates.ReservationAggregate.Reserva reserva, int aforoEfectivo)
    {
        return espacio.EsReservable
            && Edificio.PlantasReservables.Contains(espacio.Planta.Valor)
            && HorarioReservaDia.PermiteReserva(espacio.HorarioReserva, reserva.Franja)
            && reserva.NumeroAsistentes <= aforoEfectivo;
    }
}
